...
interface buildPlayerStatusProps {
  "yet": number;
  "owned": number;
  "beaten": number;
  "toplay": number;
...
export function buildPlayerStatuses(playerStatusObject: buildPlayerStatusProps): playerStatusStandardised[] | undefined {
  console.log('startingggg',playerStatusObject);
  if (isPlainObject(playerStatusObject)) {
    const finalStatuses = Object.entries(playerStatusObject).map(([key, value]) => {
      switch (key) {
        case "yet":
    ...
    console.log("finalk statuses: ", finalStatuses)
